...
@section('content')
        ...
        <form action="{{ route('mark.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ $mark->name }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Название" required="">
                </div>
    ...
    <div id="drawer-delete-product-default" class="fixed top-0 right-0 z-40 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-label" aria-hidden="true">
        ...
        <h3 class="mb-2 text-lg text-gray-500 dark:text-gray-400">Вы уверены что хотите удалить марку {{ $mark->name }}?</h3>
...
{{--                </div>--}}
                ...
                <div>
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Название</label>
                    <input type="file" name="photo" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Название" required="">
                </div>
                <div class="bottom-0 left-0 flex justify-center w-full pb-4 space-x-4 md:px-4 md:absolute">
                    <button type="submit" class="text-white w-full justify-center bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                        Добавить
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const photoInput = updateForm.querySelector('input[name="photo"]');
            ...
            updateButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    // Добавляем скрытое поле для ID марки
                    const hiddenIdInput = document.createElement('input');
                    hiddenIdInput.type = 'hidden';
                    hiddenIdInput.name = 'id';
                    hiddenIdInput.value = markId;
                    updateForm.appendChild(hiddenIdInput);
                });
            });
        });
    </script>
